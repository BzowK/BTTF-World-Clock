/*==========================================
  ...
  - To Do
    - Find out why day(now) is displaying day +1
    - Import + parse iCalendar
    - Configure Red / Orange displays with iCal data
  - 4-15-22-A
    - Implemented 12 hour option (on by default)
    - Fixed Yellow AM/PM LED pin assignment
  - 4-13-22-A
    - NTP Working
...
int year_red = 2022; // The year you wish to see on the top section
int month_red = 4;
int day_red = 15;
int hour_red = 2;
int minute_red = 00;
int pm_red = 1; // 1=yes 0=no
int year_orange = 2022; // The year you wish to see on the bottom section
int month_orange = 7;
int day_orange = 8;
int hour_orange = 5;
int minute_orange = 00;
int pm_orange = 1; // 1=yes 0=no
int ispm = 0;
...
void loop() {
  ...
  // 12 Hour Clock (Comment out below block to make 24 hour clock)
  if (hour(now()) >=12) { 
    ispm = 1;
    adjustTime(-43200);
  }
  ...
  WebSerial.print("IsPM? 1=Yes: ");
  WebSerial.println(ispm);
  WebSerial.println("*****");
  ...
  //Powersave
  if ((hour(now()) == 10) && (minute(now()) == 30) && (ispm == 1)) { // Turns displays off at given time
    green1.off();
    green2.off();
    green3.off();
    red1.off();
  ...
  else if ((hour(now()) == 7) && (minute(now()) == 00) && (ispm == 0)) { // Turns displays on at given time
    green1.on();
    green2.on();
    green3.on();
    red1.on();
  ...
  // Red Displays
  red3.printTime(month_red, day_red, true);                         
  red2.print(year_red, true);                           
  red1.printTime(hour_red, minute_red, true);        

    
  // Green Displays - NTP Time
  WebSerial.print("Day-Green: ");
  WebSerial.println(day(now()));
  green3.printTime(month(now()), day(now()), true); 
  green2.print(year(now()));
  if (hour(now())==0) {
    green1.printTime(12, minute(now()), true);
  } else { 
    green1.printTime(hour(now()), minute(now()), true);
  }
    
  // Orange Displays                     
  orange1.printTime(month_orange, day_orange, true);                        // November 25th                
  orange2.print(year_orange, true);                       // 1976
  orange3.printTime(hour_orange, minute_orange, true);  // Orange Timezone if can get it to work
}
